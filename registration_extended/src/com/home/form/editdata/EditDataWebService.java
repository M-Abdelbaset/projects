...
public class EditDataWebService {
	...
	@Path("/contacts/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public void getContacts(@PathParam("id") Integer userId, 
			@Context HttpServletRequest request) {
		
		HttpSession userSession = request.getSession();
		((EditDataBean)userSession.getAttribute("editDataBean")).getContactModels(userId);
	}
	
	@POST
	@Path("/contact")
	@Consumes(MediaType.APPLICATION_JSON)
	public void getContacts(Contact contact, @Context HttpServletRequest request) {
		
		HttpSession userSession = request.getSession();
		((EditDataBean)userSession.getAttribute("editDataBean")).addContactModel(contact);
	}
